...
#   down, left, right, or diagonally) in the  grid?
...
# Compare the current highest product to the new value
compareFive = lambda val, high: max(val,high)
...
def rlCheck(val):
 ...
 for r in range(0,20):
  for c in range(0,17):
    evalProd = int(val[r][c])
    for y in range (1,4):
#        print(val[r][c+y])
       evalProd *= int(val[r][c+y])
    highProd = compareFive(evalProd,highProd)
 return highProd
...
def udCheck(val):
 ...
 for c in range(0,20):
  for r in range(0,17):
    ...
    for y in range (1,4):
#        print(val[r][c+y])
       evalProd *= int(val[r+y][c])
    highProd = compareFive(evalProd,highProd)
 return highProd
 
#Diagonal Down check

def diagDownCheck(val):
 global highProd
 highProd = 0
 for c in range(0,17):
  for r in range(0,17):
...
rl = rlCheck(grid)
ud = udCheck(grid)
diagD = diagDownCheck(grid)
diagU = diagUpCheck(grid)
...
#print answer
print('The largest product in the grid is: %s  Found in: %s seconds') % (a,elapsed) 